// Was an inline <script> on tools/devsecops-custom.html -- externalized
// for CSP script-src (PlanDurcissement-Securite.txt P1). Must load after
// assets/script/devsecops-config.js (getDevSecOpsCategory/
// getDevSecOpsCommands) and assets/script/tools-page-boot.js
// (toggleCmd/reportHeight).
const catId = new URLSearchParams(window.location.search).get("catId");
const category = catId ? getDevSecOpsCategory(catId) : null;
document.getElementById("customCategoryTitle").textContent = category ? category.title : "DevSecOps personnalisé";

const tools = catId ? getDevSecOpsCommands(catId) : [];
