// Was an inline <script> on grc/securite/webapp/index.html -- externalized for CSP
...
const savedTheme = localStorage.getItem("/settings.html") || "standard";
  ...
  renderGrcDomains(securiteWebappIndexConfig.domains, "#securite-webapp-grid", "grc.securite.webapp");
  renderGrcCoverage(securiteWebappIndexConfig.domains, "#securite-webapp-grid", "#grc-coverage-summary");
  ...
  const securiteWebappSections = [
    { title: grcT("nav.webappSecurity"), domains: securiteWebappIndexConfig.domains, basePath: "./", keyPrefix: "grc.securite.webapp" },
  ];

  function confirmResetSection() {
    if (!confirm(grcT("grc.hub.confirmResetSection"))) return;
    resetGrcData(securiteWebappSections, () => location.reload());
  }
...
document.getElementById("btnExportJson").addEventListener("click", () => exportGrcAsJson(securiteWebappSections, "grc-securite-webapp"));
document.getElementById("btnSaveAs").addEventListener("click", () => saveGrcAsJson(securiteWebappSections, "grc-securite-webapp"));
document.getElementById("btnExportWord").addEventListener("click", () => exportGrcAsWord(securiteWebappSections, "grc-securite-webapp"));
document.getElementById("btnExportPdf").addEventListener("click", () => exportGrcAsPdf(securiteWebappSections));
